  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {filtered.length === 0 ? (
          <p className="col-span-full py-8 text-center text-venue-muted">{t("empty")}</p>
        ) : (
          filtered.map((img) => (
          <div
            key={img.id}
            className="relative aspect-square overflow-hidden rounded-xl ring-1 ring-[var(--venue-surface-border)]"
          >
            <Image
              src={img.src}
